...
def write_partial_cs(infile, cs_dir, start=0, file_count=0, samples_per_file=10):
    """
    Extract a small amount of data and write to multiple files
    
    start = starting sample number
    count = number of files to be produced
    samples_per_file: number of samples written to each file
    ...
    for i in range(file_count):
        ...
        dat = read_cs(infile, start, samples_per_file) 

        # update start for next file
        start += samples_per_file

        outfile = "%s/csfile%d.cs" % (cs_dir, i)
        print("Writing file: %s" % (outfile))

        # Open file in write mode
        with open(outfile, 'ab') as fout:
            # write header before writing data
            fout.write(hd["rawdatafile"])
            fout.write(dat)
    return
...
def cs2dada_multipass(basename, indir, outdir, nchannels, mem_lim_gb=32.0):
    """
    Get CS baseband files of the form:
...
def cs2fil_multipass(basename, cs_dir, dada_dir, fil_dir, dm, nchan, 
    ...
    # Read in and convert data to DADA file
    cs2dada_multipass(basename, cs_dir, dada_dir, nchan,
...
def cs2fil_multi(basename, cs_dir, dada_dir, fil_dir, dm, nchan, 
                 mem_lim=16.0, nproc=1, nthread=1, inc_ddm=False):
    """
    ...
    # if dada and fil dirs don't exist, make them
    if not os.path.exists(dada_dir):
